...
func (h *callbackEndpoint) start(addr, path string) {
	...
	if err != nil {
		log.Printf("cannot listen on callback endpoint, port not available %s\n", server.Addr)
		os.Exit(1)
	}
	err = ln.Close()
	if err != nil {
	...
	log.Printf("started http server for callback endpoint %s%s\n", server.Addr, path)
}
...
func (h *callbackEndpoint) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case h.shutdownSignal <- "shutdown":
		// sent successfully
	default:
		log.Printf("warning: shutdown signal channel blocked or closed\n")
	}
}
...
func (h *callbackEndpoint) renderSuccess(w http.ResponseWriter) {
	...
	if err != nil {
		http.Error(w, "Internal Server Error", http.StatusInternalServerError)
		log.Printf("error parsing success template: %v\n", err)
		return
	}
	if err := tmpl.Execute(w, nil); err != nil {
		http.Error(w, "Internal Server Error", http.StatusInternalServerError)
		log.Printf("error executing success template: %v\n", err)
		return
	}
}
